...
class StateMetadata:
    ...
    def is_violation(self) -> bool:
        """True if any metric breaches δ_crit."""
        return (
            abs(self.margin) > DELTA_CRIT or
            abs(self.entropy_delta) > MAX_ENTROPY_DELTA or
            abs(self.ward_residual) > DELTA_CRIT
        )
...
class ZenoWardProjector:
    ...
    def ward_functional(self, rho: npt.NDArray[np.float64]) -> float:
        ...
        return float(np.trace(diff @ diff.T))  # Frobenius norm squared
